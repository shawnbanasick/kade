...
const loadingsTableDataPrep = numFactors => {
  ...
  // const respondentNamesMaxLength = loadingState.respondentNamesMaxLength;
  const respondentNames = clone(coreState.respondentNames);

  // get matrix autoflag booleans
  const fSigCriterionResults = clone(rotationState.fSigCriterionResults);

  // calculate the factor groupings so they can be assigned in col defs
  const highlighting = loadingState.highlighting;
