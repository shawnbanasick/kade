...
const pushFactorsToOutputArray = (outputData, sheetNamesXlsx, colSizes) => {
  ...
  const analysisOutput2 = clone(calcState.analysisOutput);
  const analysisOutput = clone(analysisOutput2);
  const sigSortsArray = clone(calcState.sigSortsArray);
  const sortsAsNumbers = clone(calcState.sortsAsNumbers);
  const qavRespondentNames = clone(coreState.respondentNames);
  const correlationTableArray = clone(correlationState.correlationTableArray);
  ...
  const sortWeights = clone(calcState.sortWeights);
  ...
  for (let j = 0; j < analysisOutput.length; j += 1) {
    ...
    const factorWeightFactorArray = [[i18n.t("Q-Sort"), i18n.t("Weight")]];
