...
const formatConsensusArrayForDownload = (
  ...
  // getState
  const distStateThreshold1 = outputState.distStateUpperValueText;
  const distStateThreshold2 = outputState.distStateLowerValueText;
  ...
  const chartText3 =
    `${i18n.t("All Listed Statements are Non-Significant at")} ${distStateThreshold1 }${i18n.t("and Those Flagged with an are also Non Significant at")} ${distStateThreshold2})`;
  ...
  for (let j = 0; j < outputLength; j += 1) {
    ...
    line3Array.push(
      `${i18n.t("Factor")} ${statementNum} Q-SV`,
      `${i18n.t("Factor")} ${statementNum} ${chartText8}`
    );
  }
