...
const Output = () => {
        ...
        <Tab.Pane>
          ...
          <OutputDataWindow1>
            {showTableDataNotSentWarning && (
              <NoDataMessage>{t("No Data Click")}</NoDataMessage>
            )}
  ...
  const activeIndex = outputState.outputActiveTabIndex;
  showTableDataNotSentWarning = outputState.showTableDataNotSentWarning;
  const facVizContainerHeight = vizState.facVizContainerHeight;
  const facVizContainerWidth = vizState.facVizContainerWidth;
  const showNotification = outputState.notifyOutputDistStateError;
  if (showNotification) {
    notify();
  }
  return (
...
const OutputDataWindow1 = styled.div`
  display: grid;
  grid-template-rows: 100px 150px 150px 1fr 1fr;
  background-color: white;
  user-select: none;
