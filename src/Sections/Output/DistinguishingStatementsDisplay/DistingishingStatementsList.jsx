...
const DistinguishingStatementsList = () => {
  ...
  // getState
  const sortKey = clone(outputState.distStateListSortKey);
  const threshold = outputState.threshold;
  const displayData = filterDistStateListData(threshold, sortKey);
  const showFactorCorrelationsTable = outputState.showFactorCorrelationsTable;
