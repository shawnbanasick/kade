...
const SelectNumberOfCentroidFactorsButtons = () => {
  ...
  const handleExtraction = () => {
    const brownCentroids = factorState.activeTraditionalCentroidFactorButton;
    const horstCentroids = factorState.activeHorst55CentroidButton;
    const tuckerCentroids = factorState.activeTuckerMacCallumCentroidButton;
  ...
  const minNumFactors = coreState.numQsorts;
  const btnId = [1, 2, 3, 4, 5, 6, 7, 8];
  if (minNumFactors < btnId.length) {
    btnId.length = minNumFactors;
  }

  const showNumberOfCentroidFacToExtract =
    factorState.showNumberOfCentroidFacToExtract;
  const isCentroidFacSelectDisabled = factorState.isCentroidFacSelectDisabled;
