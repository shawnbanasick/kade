...
const FactorSelectButtons = props => {
  ...
  const handleSubmit = () => {
    ...
    const userSelectedRotFactors = clone(rotationState.userSelectedRotFactors);
  ...
  const handleClick = (event, baselineData) => {
    ...
    // getState
    let userSelectedRotFactors = clone(rotationState.userSelectedRotFactors);
    ...
    if (factor === "clearAllRotFacs") {
      ...
      if (!includes(userSelectedRotFactors, factor)) {
        ...
        if (userSelectedRotFactors.length === 2) {
          ...
          const factorMatrix1 = clone(factorState.factorMatrix);
  ...
  // getState
  const shouldDisplayRotFactorButtons = rotationState.shouldShowJudgeRotDiv;
  const numFactorsKeptForRotation = rotationState.numFactorsKeptForRot;
  const showRotFactorSelectWarning = rotationState.showRotFactorSelectWarning;
