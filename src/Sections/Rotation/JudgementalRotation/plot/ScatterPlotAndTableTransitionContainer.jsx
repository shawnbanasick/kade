...
const ScatterPlotAndTableTransitionContainer = props => {
  ...
  const showScatterPlotTableDiv = rotationState.showScatterPlotTableDiv;
  const degreesText = `${rotationState.rotationDegrees}\u00B0`;
  const data = clone(rotationState.newRotationVectors);
  const leftContWidth = getWidthHeight();
  const colDefs = clone(rotationState.rotColDefsFactorTable);
  const rowData = clone(rotationState.rotRowDataFactorTable);
