...
const processBlob = (data) => {
  ...
  if (lines3.length < 2) {
    ...
    inputState.setState({ errorStackTrace: "Error in 'processSheetsCsv' function." });
    inputState.setState({
  ...
  newArray.forEach((item) => {
    ...
    labelStrippedArray.push(item.pop().replace('sort:', '').trim());
  });
  ...
  if (!sortsArray[0][1]) {
    ...
    inputState.setState({ errorMessage: "Can't find the respondent names in the file!" });
    inputState.setState({ showErrorMessageBar: true });
    inputState.setState({ errorStackTrace: "Error in 'processSheetsCsv' function." });
    inputState.setState({
      extendedErrorMessage:
  ...
  if (!sortsArray[0][0]) {
    // throw new Error("Can't find participant names or Q sorts in the file!");
    inputState.setState({ errorMessage: "Can't find the Q sorts in the file!" });
    inputState.setState({ showErrorMessageBar: true });
    inputState.setState({ errorStackTrace: "Error in 'processSheetsCsv' function." });
    inputState.setState({
      extendedErrorMessage:
  ...
  if (hasInputError === false) {
    ...
    inputState.setState({ dataOrigin: 'sheets' });
    inputState.setState({ areQsortsLoaded: true });
    inputState.setState({ isLoadSheetsCsvButtonGreen: true });
