...
const DownloadExampleT2 = () => {
  ...
  const handleClick = async () => {
    console.log('DownloadExampleT2 button clicked');
    ...
    try {
      ...
      console.error('Failed to export Excel T2 Example file:', error);
    }
  };

  return (
    <NewLoadButton
      className={`w-87.5 h-18.75 ml-10 rounded-lg border-2 border-black bg-[#F0F0F0] hover:bg-[#E0E0E0] active:bg-[#D0D0D0]`}
      onClick={handleClick}
    >
      <div
      ...
      >
        ...
        <div className="font-sans text-lg font-bold">{t('Download Type 2 Template')}</div>
      </div>
    </NewLoadButton>
  );
};

export default DownloadExampleT2;
