...
const pushFactorsToOutputArray = (outputData, sheetNamesXlsx, colSizes) => {
  ...
  const results = getLoadingState("currentLoadingsTable");
  ...
  if (problemFactorsArray.length > 0) {
    ...
    outputState.factorsWithoutLoading = problemFactorsArray;
    outputState.showNoLoadingsFlaggedWarningModal = true;
  ...
  const analysisOutput2 = getCalcState("analysisOutput");
  const analysisOutput = clone(analysisOutput2);
  const sigSortsArray = getCalcState("sigSortsArray");
  const sortsAsNumbers = getCalcState("sortsAsNumbers");
  const qavRespondentNames = getCoreState("respondentNames");
  const correlationTableArray = getCorrelationState("correlationTableArray");
  const tableHeader = clone(qavRespondentNames);
  tableHeader.unshift("");
  ...
  const sortWeights = getCalcState("sortWeights");
  ...
  for (let j = 0; j < analysisOutput.length; j += 1) {
    ...
    const factorWeightFactorArray = [
      [i18n.t("Nm"), i18n.t("Q-Sort"), i18n.t("Weight")]
    ];
    ...
    for (let m = 0, mLen = analysisOutput[0].length; m < mLen; m += 1) {
      ...
      for (let s = 0; s < sLen; s += 1) {
        tempObj[`${i18n.t("Raw Sort")} ${sigSortsArray[j].SigSorts[s]}`] =
          rawSorts[j][s][m];
    ...
    synFactorArray1 = clone(synFactorArray);
  ...
  calcState.factorWeightFactorArrayHolder = factorWeightFactorArrayHolder;
  calcState.miniCorrelationArrayHolder = miniCorrelationArrayHolder;
  calcState.synFactorArray1Holder = synFactorArray1Holder;
  calcState.matchCount = matchCount;
  calcState.compositeFactorMasterArray = compositeFactorMasterArray;
