...
const calculateFactorLoadings = (dataArray) => {
  ...
  const factorLoads1 = calculateFactor(reflectedArray1, reflectedArrayColumnTotals);
  const subtractArray = removeCorrelations(reflectedArray1, factorLoads1);
  const undoPositiveManifold = undoReflection(subtractArray, factorLoads1, reflectedRowCol);
