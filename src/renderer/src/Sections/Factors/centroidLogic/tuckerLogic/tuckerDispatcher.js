...
const tuckerDispatcher = () => {
  ...
  const dataArray = getCorrelationState('correlation5Calcs');
  ...
  // const numState = getCoreState("numStatements");
  const numQsorts = getCoreState('numQsorts');

  const tuckerCalculations2 = tuckerMain(numCentroidFactors, dataArray, numQsorts);

  const tuckerCalculations = clone(tuckerCalculations2);
  const formattedScreePlotData = formatScreePlotData(tuckerCalculations.explainVarandEigens[0]);
  const cumulativeVar = calcCumulativeVar(tuckerCalculations.explainVarandEigens[1]);
  ...
  const projectLogText2 = i18n.t('Tucker and MacCallum Centroid Factors Extracted');
