...
const pcaDispatch = () => {
  ...
  const X = getCorrelationState("correlation5Calcs");
  ...
  const formattedEigenCum = clone(eigenValuesCumulPercentArray);
  formattedEigenCum.unshift(i18n.t("Cumulative Explained Variance"));
  const formattedEigenPer = clone(eigenValuesAsPercents);
  formattedEigenPer.unshift(i18n.t("percent explained variance"));

  // create data for scree plot
  const eigenData = clone(eigenValuesSorted);
